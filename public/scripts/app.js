$(function () {
  const createNewTweets = {
    ...
    createTweetElement: function (tweetObject) {
      ...
      const likeCount = $('<p>')
        .addClass('likes-counter')
        .text(tweetObject.likeCount);
  ...
  const handlingTweets = {
    ...
    newTweetSubmit: function (event) {
      ...
      if (
        ...
        $.post("/tweets", tweetTextSerialized, handlingTweets.loadTweets);
        $("form textarea").val("");
      } else {
    ...
    newLike: function (event) {
      ...
      const tweetId = target.parent().parent().parent().parent().data('id');
      $.post("tweets/likes", tweetId, handlingTweets.loadTweets);
    },
    /**
